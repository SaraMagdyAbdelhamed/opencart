...
function Product_List_View($conn, $db_prefix,$request) {
    ...
    {
       $sql .= " AND " . $db_prefix . "product_description.language_id=1"; 
    }
    ...
    {
         $q .= " AND " . $db_prefix . "attribute_description.language_id=1";
    }
        $t = array();
        if (DB_DRIVER == 'mysqli') {
            $data = $conn->query($q);
    ...
    {
        $q_color .= " AND " . $db_prefix . "attribute_description.language_id=1";
    }
        if (DB_DRIVER == 'mysqli') {
...
function Store_Depts($conn, $db_prefix,$request) {
    ...
    {
       $wh .= " AND " . $db_prefix . "category_description.language_id=1"; 
    }
    if (isset($request['ID']))
...
function Store_Display($conn, $db_prefix, $product_id,$request) {
    ...
    {
       $sql .= " AND " . $db_prefix . "category_description.language_id=1"; 
    }
    ...
    {
      $q .= " AND " . $db_prefix . "category_description.language_id=1";  
    }
...
function Store_Product($conn, $db_prefix,$request) {
    ...
    {
        $sql .= " AND " . $db_prefix . "attribute_description.language_id=1";
    }
...
function Shopping_cart($conn, $db_prefix,$request) {
    ...
    if ($query_token->num_rows == 1) {
        $user_id=$query_token->fetch_assoc()['user_id'];
        $sql = "SELECT * FROM {$db_prefix}cart WHERE customer_id='".$user_id."'";
        $query = $conn->query($sql);
        if ($query->num_rows > 0 ) {
            $temp = array();
            $Final_Json = array();
...
function checkout($conn,$db_prefix,$data)
{
    ...
    if ($query_token->num_rows == 1) {
        ...
        $cart=$conn->query("SELECT ".$db_prefix."product.product_id as 'product_id' , ".$db_prefix."product.price as 'price' ,".$db_prefix."product.model as 'model' ,".$db_prefix."product_description.name as 'name' ,".$db_prefix."cart.quantity as 'quantity' From ".$db_prefix."cart  
...
function Json_Basic_Data($output,$Others_Data,$comment_list,$discount) {
    ...
    $Arr['image'] = "$output[image]";
    $Arr['content']="";
    $Arr['Key'] = "$output[Key]";
    $Arr['link_share'] = "$output[link_share]";
    $Arr['created_at'] = "$output[created_at]";
    $Arr['duration']="";
    // $Arr['price']="$Price";
    // $Arr['rate']="$Rate";
    // $Arr['price_currency']="$Price_Currency";
    $Arr['Links'] = "$output[Links]";
    
    if (count($output['images']) == 0)
        $output['images'] = array();
    if (count($output['videos']) == 0)
        $output['videos'] = array();
    $Arr['media'] = $output['images'];
...
function Json_Others_Data($output,$Others_Data) {
    // return $Others_Data;

    $array['key']='keyvalue';
    $array['group_name']="Key Value Fields";
    if(count($Others_Data) > 0 || isset($output['price']) || isset($output['rate']) || isset($output['price_currency']) || isset($output['has_discount']) || $output['has_sub'])
{
    if(count($Others_Data) > 0 )
    {
    //      if(isset($output['price']))
    // {
    //     $array['value'][]=array("id"=>1,"parameter"=>"","name"=>'price',"value"=>array("value_type"=>"normal","value_string"=>$output['price']),"dkv_id"=>"","title"=>"","des"=>"","setting_id"=>"");
    // }
    //  if(isset($output['rate']))
    // {
    //     $array['value'][]=array("id"=>1,"parameter"=>"","name"=>'rate',"value"=>array("value_type"=>"normal","value_string"=>$output['rate']),"dkv_id"=>"","title"=>"","des"=>"","setting_id"=>"");
    // }
        foreach ($Others_Data as $key => $value) {
        $arr[]=array("id"=>$value['ID'],"key"=>$value['Key'],"value"=>$value['Value']);
    }
    $array['value'][]=array("id"=>'',"parameter"=>"","name"=>"specifications","value"=>array("value_type"=>"array","value_array"=>$arr),"dkv_id"=>"","title"=>"","des"=>"","setting_id"=>"");
    }
    ...
    {
        $array['value'][]=array("id"=>'',"parameter"=>"has_discount","name"=>'has_discount',"value"=>array("value_type"=>"normal","value_string"=>$output['has_discount']),"dkv_id"=>"","title"=>"","des"=>"","setting_id"=>"");
    }
...
function Category_Product($conn,$db_prefix,$request)
{
 $Category_List = array();
    ...
    {
        $sub=$conn->query("SELECT count(category_id) as 'count' FROM ".$db_prefix."category WHERE parent_id = '".$Category['ID']."'");
...
function submit_review($conn,$db_prefix,$request)
{
    ...
    $sql_review = $conn->query("INSERT INTO " . $db_prefix . "review SET customer_id = '" . $user_id . "', product_id = '".$request['product_id']."',text = '".$request['review']."',rating = '".$request['rating']."' , date_added = NOW()");
            ...
            $id = $conn->insert_id;
            $review=$conn->query("SELECT * FROM ".$db_prefix."review WHERE review_id='".$id."'");
         $res=$review->fetch_assoc();
         $rate=$res['rating'];
...
{
    if(isset(getallheaders()["access-token"]) && getallheaders()["access-token"] != "")
    {
       $query_token = $conn->query("SELECT * FROM ". $db_prefix ."api_tokens WHERE api_token='".getallheaders()["access-token"]."'");
        if ($query_token->num_rows > 0) 
        {
...
function wishlist($conn,$db_prefix,$request)
{
    ...
    if ($query_token->num_rows == 1) {
        ...
        else
        {
            ...
            output($Final_Json);
        //      $userdata =  array("status" => array("code"=>200,"message"=>"success","error_details"=>array()), "content" => array(array("products"=>$products)));
        // return json_encode($userdata);
        }
        else {
        $userdata =  array("status" => array("code"=>204,"message"=>"No data","error_details"=>array( "cart is empty"),"validation_errors"=>array()), "content" => array());
...
function product_filters($conn,$db_prefix,$request)
{
    ...
    $sql = "SELECT DISTINCT " . $db_prefix . "product.product_id as 'ID', date_added, viewed, image as 'Img'," . $db_prefix . "product.quantity as 'Quantity',name as 'Title',".$db_prefix."product.price as 'Price',description as 'Description' 
        FROM " . $db_prefix . "product 
        INNER JOIN " . $db_prefix . "product_description ON " . $db_prefix . "product_description.product_id = ". $db_prefix . "product.product_id ";
    ...
    if(isset($request['price']) && $request['price'] != "" && isset($request['price_operator']) && $request['price_operator'] != "")
    {
        switch ($request['price_operator']) 
        {
            case '=':
