...
class Base64ToImageService
{
    // protected $upload_path;
       protected $base_path="C:/xampp\htdocs\opencart\upload/";
//    protected $base_path = "/home/evilcell/public_html/secure_bridge/public/";
    ...
    public static function convert($base64_string ,$upload_path)
    {
        ...
        if(isset($base64_string) && is_string($base64_string) && strlen($base64_string))
        {
            ...
            if (!file_exists($base_path.$get_folder[0])) {
                 // return "exist";
                    mkdir($base_path.$get_folder[0], 0777, true);
                }
                // return $upload_path;
            $output_file = self::generateImage($upload_path, $base64_string);
            // return $output_file;
            $file_stream = fopen($base_path.$output_file, 'wb');
...
//                    $result = explode($this->upload_path, $output_file)[1];
    ...
    {
         // return $upload_path;
        $image_random_name = self::getRandomString().'.'.self::getBase64ImageExtension($base64_string);
        if(file_exists($base_path.$upload_path.$image_random_name))
        {
           self::generateImage($upload_path, $base64_string);
        }
