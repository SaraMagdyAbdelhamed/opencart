<?php
class ControllerRestapiPaymentG2APay extends Controller {
	public function index() {
		$this->load->language('payment/g2apay');

		$data['button_confirm'] = $this->language->get('button_confirm');

		$data['action'] = $this->url->link('restapi/payment/g2apay/checkout', '', true);
	...
	public function checkout() {
			...
			if ($this->config->get($result['code'] . '_status')) {
				...
				if (isset($order_data['totals'][$i])) {
					if (strstr(strtolower($order_data['totals'][$i]['code']), 'total') === false) {
						...
						$item->url = $this->url->link('restapi/common/home', '', true);
						$items[] = $item;
					}
		...
		foreach ($ordered_products as $product) {
			...
			$item->url = $this->url->link('restapi/product/product', 'product_id=' . $product['product_id'], true);
			$items[] = $item;
		}
		...
		$fields = array(
			...
			'url_failure' => $this->url->link('restapi/checkout/failure'),
			'url_ok' => $this->url->link('restapi/payment/g2apay/success'),
		...
		if ($response_data === false) {
			$this->response->redirect($this->url->link('restapi/payment/failure', '', true));
		}
