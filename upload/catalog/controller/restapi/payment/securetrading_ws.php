<?php
class ControllerRestapiPaymentSecureTradingWs extends Controller {
	...
	public function process() {
		...
		if ($order_info) {
			if ($this->config->get('securetrading_ws_3d_secure')) {
				...
				$merchant_node->addChild('termurl', $this->url->link('restapi/payment/securetrading_ws/threedreturn', '', true));
					...
					if ($response_xml->response['type'] == 'THREEDQUERY') {
						...
						if ($error_code == 0) {
							...
							if ($enrolled == 'Y') {
								...
								$json['md'] = $md;
								$json['pareq'] = $pareq;
								$json['term_url'] = $this->url->link('restapi/payment/securetrading_ws/threedreturn', '', true);
							} else {
								$requestblock_xml = new SimpleXMLElement('<requestblock></requestblock>');
								$requestblock_xml->addAttribute('version', '3.67');
								$requestblock_xml->addChild('alias', $this->config->get('securetrading_ws_username'));
		...
		if ($response !== false) {
			...
			if ($response_xml->response['type'] == 'AUTH') {
				...
				if ($error_code == 0) {
					...
					$json['redirect'] = $this->url->link('restapi/checkout/success');
					$json['status'] = 1;
				} else {
					$this->model_payment_securetrading_ws->updateOrder($order_id, $this->config->get('securetrading_ws_declined_order_status_id'));
				...
				$this->model_payment_securetrading_ws->updateOrder($order_id, $this->config->get('securetrading_ws_failed_order_status_id'));
