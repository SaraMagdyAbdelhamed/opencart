<?php
class ControllerRestapiPaymentPPExpress extends Controller {
	public function index() {
		...
		$data['continue'] = $this->url->link('restapi/payment/pp_express/checkout', '', true);
	...
	public function express() {
		...
		$data = array(
			...
			'MAXAMT'             => $max_amount,
			'RETURNURL'          => $this->url->link('restapi/payment/pp_express/expressReturn', '', true),
			'CANCELURL'          => $this->url->link('restapi/checkout/cart', '', true),
	...
	public function expressReturn() {
		...
		$this->response->redirect($this->url->link('restapi/payment/pp_express/expressConfirm', '', true));
	}
	...
	public function expressConfirm() {
		...
		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('restapi/payment/pp_express/express'),
			'text' => $this->language->get('text_title')
		);

		$data['breadcrumbs'][] = array(
			'href' => $this->url->link('restapi/payment/pp_express/expressConfirm'),
		...
		$data['action'] = $this->url->link('restapi/payment/pp_express/expressConfirm', '', true);
		...
		foreach ($products as $product) {
			...
			$data['products'][] = array(
				...
				'remove'                => $this->url->link('restapi/checkout/cart', 'remove=' . $product['cart_id']),
			...
			if (!empty($shipping_address)) {
				...
				if (!empty($results)) {
					...
					if (!empty($quote_data)) {
						...
						$data['action_shipping'] = $this->url->link('restapi/payment/pp_express/shipping', '', true);
					} else {
						unset($this->session->data['shipping_methods']);
						unset($this->session->data['shipping_method']);
		...
		$data['action_confirm'] = $this->url->link('restapi/payment/pp_express/expressComplete', '', true);
	...
	public function expressComplete() {
		...
		if ($this->cart->hasShipping()) {
			...
			if (empty($shipping_address)) {
				$redirect = $this->url->link('restapi/checkout/checkout', '', true);
			}
		...
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$redirect = $this->url->link('restapi/checkout/cart');
		}
		...
		if ($redirect == '') {
			...
			$paypal_data = array(
				...
				'PAYMENTREQUEST_0_NOTIFYURL' => $this->url->link('restapi/payment/pp_express/ipn', '', true),
				'RETURNFMFDETAILS'           => 1
			);
			...
			if ($result['ACK'] == 'Success') {
				...
				$this->response->redirect($this->url->link('restapi/checkout/success'));
	...
	public function checkout() {
		...
		$data = array(
			'METHOD'             => 'SetExpressCheckout',
			'MAXAMT'             => $max_amount,
			'RETURNURL'          => $this->url->link('restapi/payment/pp_express/checkoutReturn', '', true),
			'CANCELURL'          => $this->url->link('restapi/checkout/checkout', '', true),
			'REQCONFIRMSHIPPING' => 0,
			'NOSHIPPING'         => $shipping,
			'LOCALECODE'         => 'EN',
			'LANDINGPAGE'        => 'Login',
		...
		if ((string)$response == "VERIFIED")  {
			...
			if (isset($this->request->post['txn_type'])) {
				...
				if ($this->request->post['txn_type'] == 'recurring_payment') {
					...
					$this->model_payment_pp_express->log($recurring, 'IPN data');
	...
	public function shipping() {
		...
		$this->response->redirect($this->url->link('restapi/payment/pp_express/expressConfirm'));
	}
