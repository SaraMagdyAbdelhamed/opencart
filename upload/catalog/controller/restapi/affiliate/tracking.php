...
class ControllerAffiliateTracking extends Controller {
	public function index() {
		if (!$this->affiliate->isLogged()) {
			$this->session->data['redirect'] = $this->url->link('restapi/affiliate/tracking', '', true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('restapi/affiliate/tracking', '', true)
		);
		...
		$data['continue'] = $this->url->link('restapi/affiliate/account', '', true);
		...
		if (isset($this->request->get['filter_name'])) {
			...
			foreach ($results as $result) {
				$json[] = array(
					...
					'link' => str_replace('&amp;', '&', $this->url->link('restapi/product/product', 'product_id=' . $result['product_id'] . '&tracking=' . $this->affiliate->getCode()))
				);
			}
		}
